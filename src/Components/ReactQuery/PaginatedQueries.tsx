    ...
    <>
      ...
      >
        <Input
          ...
          value={colorVal}
          onChange={(e: any) => setColorVal(e.target.value)}
        />
      ...
      <Box sx={{ height: 400, width: "100%" }}>
        <DataGrid
          ...
          onPageChange={(newPage: number) => {
            console.log("changing page to --->>", newPage, 7984);
            setPage(newPage + 1);
          }} // when changing page
          pageSize={pageSize} //no of data being shown currently
          rowsPerPageOptions={[2, 5, 10]} // dropdown allowing to show no of pages
          onPageSizeChange={(newPageSize: number) => setPageSize(newPageSize)} //on changing no of rows to show
      ...
      {actionModalOpen && (
        <ActionModal color={actionModalOpen} handleClose={handleModalClose} />
      )}
      {isFetching && "Loading"}
    </>
  );
