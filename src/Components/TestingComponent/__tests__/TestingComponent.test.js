...
describe("Transaction list test case", () => {
  ...
  it.skip("Should show loader over table till data loads", () => {
    useTransactionSearchContext.mockReturnValue({
      isLoading: true,
      transactionData: [],
      pagination: {},
    ...
    expect(
      container.getElementsByClassName("ant-spin-nested-loading")[0]
    ).toBeInTheDocument();
  });

  it.skip("Should render the component properly with data", async () => {
    useTransactionSearchContext.mockReturnValue({
      isLoading: false,
      transactionData: [
        {
    ...
    await act(async () => await render(<TransactionList />));
    await waitFor(
      async () => await screen.findByText("SHIPPING_FEE_ITEM_LEVEL")
    );
    await waitFor(async () => {
