...
export const useFetchColorHook = (page) => {
  return useQuery(["colors", page], () => fetchColors(page), {
...
export const useAddColorHook = (page) => {
  ...
  return useMutation(addColor, {
    ...
    onMutate: async (color) => {
      // color refers to the argument being passed to the mutate function
    ...
    onSuccess: (response, variables, context) => {
      queryClient.setQueryData(["colors", page], (oldQueryData) => {
        if (oldQueryData) {
          ...
          return {
            ...oldQueryData,
            data: oldQueryData.data.map((data) =>
              data.label === variables.label ? response.data : data
            ),
          };
        } else {
          return {
